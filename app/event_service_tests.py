...
class EventServiceTests(unittest.TestCase):
    ...
    def test_create_post_event(self):
        ...
        # Create a post event
        self.event_service.create_event(1, test_user.user_id)
        ...
        self.assertEqual(created_event[1], 1)
    ...
    def test_create_comment_event(self):
        ...
        # Get the created event from the DAO
        events = self.eventDAO.get_events()
        created_event = events[-1]

        # Assert that the event type and user ID are correct
        self.assertEqual(created_event.type_of_action, 2)
        self.assertEqual(created_event.user_id, test_user.user_id)
